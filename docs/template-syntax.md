...
- Context aliases available in templates: `global`, `objects` (`global.objects`), `classes` (`global.classes`), and `stack` (plain map of all objects). `canonical` is also available on global builds.
